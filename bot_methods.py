...
def fixed_mini_max(game):
    ...
    for pos in game.putlist:
        ...
        for next_pos in next_poslist:
            ...
            next_score = static_scores.calc_score(game, next_board)
...
def max_only(game):
    ...
    for pos in game.putlist:
        board = game.put_stone(pos, game.turn, test=True) # 1手先置きした盤面
        score = static_scores.calc_score(game, board)
...
def soft_max(game):
    ...
    def playout(game, board):
        ...
        while is_game_end == False:
            ...
            for pos in putlist:
                ...
                scores.append(static_scores.calc_score(game, test_board) * turn_flg)
    ...
    for pos in game.putlist:
        board = game.put_stone(pos, game.turn, test=True)
        score.append(static_scores.calc_score(game, board))
